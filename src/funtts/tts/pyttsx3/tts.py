...
class Pyttsx3TTS(BaseTTS):
        ...
        try:
            ...
            if voices:
                ...
                if selected_voice:
                    ...
                    logger.info(f"选择语音: {selected_voice.name} ({selected_voice.id})")
                else:
    ...
    def _select_voice(self, voices):
        ...
        # 按名称或ID匹配
        for voice in voices:
            if (self.voice_name in voice.name or 
                self.voice_name == voice.id or
                self.voice_name.lower() in voice.name.lower()):
                return voice
    ...
    def _generate_subtitles(self, request: TTSRequest, audio_file: str, duration: float):
        """生成字幕文件
    ...
    def list_voices(self, language: Optional[str] = None) -> List[VoiceInfo]:
        ...
        try:
            ...
            for i, voice in enumerate(voices):
                ...
                # 语言过滤
                if language:
                    if not any(lang.lower().startswith(language.lower()) 
    ...
    def _infer_gender_from_name(self, voice_name: str) -> str:
        ...
        # 常见的性别标识
        if any(indicator in name_lower for indicator in ['female', 'woman', 'girl', 'she']):
            return 'female'
        ...
        male_names = ['david', 'michael', 'john', 'robert', 'william', 'james', 'richard']
    ...
    def is_voice_available(self, voice_name: str) -> bool:
        ...
        try:
            ...
            return any(voice.name == voice_name or voice.display_name == voice_name 
                      for voice in voices)
        except Exception:
            return False
