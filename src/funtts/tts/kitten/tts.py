...
class KittenTTS(BaseTTS):
    ...
    def _calculate_audio_duration(self, audio_data) -> float:
        ...
        try:
            if hasattr(audio_data, 'shape'):
                ...
                # 默认估算
                return len(audio_data) / self.sample_rate if hasattr(audio_data, '__len__') else 0.0
    ...
    def list_voices(self, language: Optional[str] = None) -> List[VoiceInfo]:
        ...
        try:
            ...
            # 获取模型支持的语音列表
            voices = self.model.get_available_voices() if hasattr(self.model, 'get_available_voices') else []
            ...
            for voice in voices:
                ...
                voice_info = VoiceInfo(
                    ...
                    display_name=getattr(voice, 'display_name', getattr(voice, 'name', 'Unknown')),
                    language=voice_lang,
